    ...
    <body>
        ...
        <main class="flex flex-col items-center gap-8 px-[100px] py-[25px] max-[1260px]:px-[15px] max-[775px]:px-[5px]">
            <section class="w-full max-w-[1000px] rounded-[25px] bg-[var(--ContentBoxBackground)] px-8 py-7 text-center shadow-[10px_10px_20px_black] max-[775px]:px-4">
                ...
                <p class="mt-4 font-[Arial,Helvetica,sans-serif] text-lg leading-7 text-white/90 max-[775px]:text-base">
                    Trage die Kontaktdaten und die Kinder ein. Der vorhandene Muttizettel wird direkt im Browser ausgefüllt und heruntergeladen.
                </p>
            </section>

            <form id="MuttizettelForm" class="w-full max-w-[1000px] rounded-[25px] bg-[#2a2a2a] p-6 shadow-[10px_10px_20px_black] max-[775px]:p-4">
                <div class="grid grid-cols-3 gap-4 max-[850px]:grid-cols-1">
                    <label class="flex flex-col gap-2 font-[Arial,Helvetica,sans-serif] text-white/90">
                        Name Erziehungsberechtigte/r
                        <input name="parent_name" type="text" class="rounded-[10px] border border-white/10 bg-[#151515] p-3 text-white outline-none focus:border-[#73ffff]" />
                    </label>
                    <label class="flex flex-col gap-2 font-[Arial,Helvetica,sans-serif] text-white/90">
                        Telefonnummer
                ...
                <div class="mt-8 rounded-[15px] border border-[#00aaaa] bg-[#151515] p-4 font-[Arial,Helvetica,sans-serif] text-white/90">
                    Die vier Kinder werden direkt in die Vorlage eingetragen. Nicht benötigte Felder können einfach leer bleiben.
                </div>
        ...
        <script>
            ...
            function fillPdfFormFields(pdfDoc, font, data, children) {
                ...
                const parentName = data.get('parent_name')?.trim();
                ...
                filledFields += fillFormField(fields, 'Name', parentName) ? 1 : 0;
                filledFields += fillFormField(fields, 'Telefon', phone) ? 1 : 0;
                filledFields += fillFormField(fields, 'Termin', eventDate) ? 1 : 0;
                filledFields += fillFormField(fields, 'Datum', placeDate) ? 1 : 0;

                children.slice(0, 4).forEach((child, index) => {
                    const childNumber = index + 1;
                    filledFields += fillFormField(fields, `Kind ${childNumber}`, child.name) ? 1 : 0;
                    filledFields += fillFormField(fields, `Kind ${childNumber} Geburtstag`, child.birthday) ? 1 : 0;
                });
                ...
                if (filledFields === 0) {
                    const valuesByVisualOrder = [
                        ...
                        eventDate,
                        placeDate,
                        ...children.slice(0, 4).map((child) => child.name),
                        ...children.slice(0, 4).map((child) => child.birthday),
                    ].filter(Boolean);
            ...
            function drawPdfOverlay(pdfDoc, font, data, children) {
                ...
                drawText(page, font, data.get('parent_name')?.trim(), 128, 279, 10);
                drawText(page, font, data.get('phone')?.trim(), 148, 260, 10);
                drawText(page, font, formatDate(data.get('event_date')), 155, 241, 10);
                drawText(page, font, data.get('place_date')?.trim(), 10, 12, 8);

                const childPositions = [
                    { x: 12, y: 118, birthdayY: 35 },
                    { x: 137, y: 118, birthdayY: 35 },
                    { x: 261, y: 118, birthdayY: 35 },
                    { x: 384, y: 118, birthdayY: 35 },
                ];
